...
try {
    if (areFirebaseKeysValid(firebaseConfig)) {
        app = initializeApp(firebaseConfig);
        db = getFirestore(app);
        auth = getAuth(app);
    } else {
        throw new Error("One or more Firebase environment variables are missing or invalid. Please check your Vercel project settings.");
    }
} catch (error) {
...
const HelpIcon = () => <Icon path="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9 5.25h.008v.008H12v-.008z" />;
...
const HomePage = ({ setCurrentPage, plants, isLoading }) => (
    <div className="text-center animate-fade-in">
        ...
        <div className="my-8 bg-white p-8 rounded-xl shadow-lg">
            ...
            <p className="text-gray-700 mb-4">As a registered plant nursery right here in Florida, we're passionate about connecting people with the joy of plants. We're a small team with big hearts, dedicated to providing you with quality greenery and the support you need to help it thrive.</p>
            <button onClick={() => setCurrentPage('products')} className="bg-yellow-400 hover:bg-yellow-500 text-green-800 font-semibold py-3 px-6 rounded-lg shadow-md transition-transform duration-150 hover:scale-105">Explore Our Green Gang</button>
        </div>
    ...
    return (
        <div className="bg-white p-8 rounded-xl shadow-xl animate-fade-in">
            ...
            <div className="mt-6 text-right">
                ...
                <button onClick={() => { alert('Checkout not implemented.'); clearCart(); }} className="mt-4 bg-yellow-400 text-green-800 font-bold py-2 px-8 rounded-lg shadow-lg">Checkout</button>
            </div>
        </div>
    );
};
